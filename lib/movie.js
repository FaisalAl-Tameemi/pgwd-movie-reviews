...
const Movie = dbHelper.initCollection('movies', {
  reset: true
});
...
const build = (data) => {
  return {
    ...
    reviews: [],
    save: function() {
      Movie.update(this.id, this);
    },
    remove: function() {
      Movie.remove(this.id);
    },
...
module.exports = {
  ...
  create,
  find,
  remove: Movie.remove // just a reference
};
